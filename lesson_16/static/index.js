...
// Глобальная переменная для хранения количества друзей
var friendsCount;

function getFriends() {
  // Используем глобальную переменную friendsCount
  friendsCount = Math.floor(Math.random() * 100);
  $("#friendsCount").text(friendsCount);
}

function addFriend() {
  var btn = $("#addFriendBtn");
  if (btn.prop("disabled")) {
    // Кнопка уже была нажата
    return;
  }
  btn.text("Очікується підтвердження");
  btn.prop("disabled", true);
  btn.css("background-color", "rgb(32,77,36)");
  // Используем глобальную переменную friendsCount и увеличиваем на 1
  friendsCount++;
  $("#friendsCount").text(friendsCount);
}
